...
const BookingForm = ({currentUser, paymentIntent}: Props) => {
  ...
  const {mutate: bookRoom, isLoading} = useMutation(apiClient.createBooking, {
    onSuccess: () => {
      showToast({ message: "Booked successfully", type: "SUCCESS" })
    },
    onError: () => {
      showToast({ message: "Error booking room", type: 'ERROR' })
    }
  })
  ...
  return (
    <form className="grid grid-cols-1 gap-5 rounded-lg border border-slate-300 p-4" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="flex justify-end">
        <button type="submit" className="bg-blue-600 text-white p-2 font-bold hover:bg-blue-500 text-md disabled:bg-gray-500" disabled={isLoading}>
